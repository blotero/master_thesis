...
\begin{equation}
  ...
  \mathbb{E}_{k} \Bigg\{
    \mathbf{Y}_r \circ \left(
      \frac{\mathbf{1}_{W\times H \times K} - f(\mathbf{X};\theta)^{\circ
  q}}{q} \right) \Bigg\}
\end{equation}

\begin{equation}
  \mathcal{L}_{prior} = \frac{\mathbf{1}_{W\times H} -
  \left(\frac{1}{K} \mathbf{1}_{W\times H}\right)^{\circ q}}{q}
\end{equation}
...
\begin{equation}
  \begin{split}
    ...
    \mathbf{\Lambda}_r(\mathbf{X};\theta)) = \mathbb{E}_{r,w,h} \Bigg\{
      \\
      \Lambda_r(\mathbf{X};\theta) \circ
      \mathcal{L}_{GCE} +
      \left(\mathbf{1}_{W\times H} -
      \Lambda_r(\mathbf{X};\theta)\right) \circ \mathcal{L}_{prior};
      \\
...
\begin{itemize}
  \item Treat all annotators equally without considering their varying expertise
