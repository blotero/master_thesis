...
\begin{figure}[H]
  ...
  \caption{Example evaluation of the proposed model in some given
    epoch (15/50) of training, which demonstrates a quick convergence
    ...
    to right. learning rate = 0.00065,
    $q = 0.66$, $\tilde{C} = 0.64$, $\alpha_{centroid} = 3.6$,
  $\alpha_{entropy} = 2.3$, $\alpha_{\lambda} = 1.7$, $\beta = 0.77$.}
...
\begin{figure}[H]
  ...
  width=\textwidth]{Cap5/Figures/labelers_classes_distribution_histologies.jpeg}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{Cap5/Figures/crowd_seg_annotated_vs_dice.png}
...
This is likely due to the way in which the labels were created
within the HistomicsTK metadata and then exported and cropped to
the desired size.
...
In table \ref{tab:experimental_results} the dice scores for different
annotation types are shown. It can be seen that the pixel-wise
annotation type achieves the highest dice score for the Tumor class,
while the scalar annotation type achieves the highest dice score for
the Other and Stroma classes.

In figure \ref{fig:tnbc_all_training_curves} the training curves for
different model variants are shown. Similar to the experiments seen
on the Oxford-IIIT Pet Dataset, the features-based model has better
loss convergence than the scalar model and better numerical stability
than the pixel-wise model.
...
\begin{figure}[H]
  ...
  \begin{subfigure}[b]{0.80\textwidth}
    \centering
    \includegraphics[width=\textwidth]{Cap5/Figures/crowd_seg_features_curves.png}
    \caption{Features-based model}
    \label{fig:tnbc_features_training_curves}
  \end{subfigure}
  \hfill
  \begin{subfigure}[b]{0.80\textwidth}
    \centering
    \includegraphics[width=\textwidth]{Cap5/Figures/crowd_seg_scalar_curves.png}
    \caption{Scalar model}
    \label{fig:tnbc_scalar_training_curves}
  \end{subfigure}
  \hfill
  \begin{subfigure}[b]{0.80\textwidth}
    \centering
    \includegraphics[width=\textwidth]{Cap5/Figures/crowd_seg_pixel_curves.png}
    \caption{Pixel-wise model}
    \label{fig:tnbc_pixel_training_curves}
  \end{subfigure}
  \caption{Training curves comparison for different model variants on
    the \gls{TNBC}
    (Crowd-Seg) dataset: (a) baseline model, (b) features-based
  model, (c) scalar model, and (d) pixel-wise model.}
  \label{fig:tnbc_all_training_curves}
\end{figure}

\begin{table}[h]
  \centering
  \caption{Experimental Results: Dice Scores for Different Annotation
  Types in the \gls{TNBC} (Crowd-Seg) dataset.}
  \label{tab:experimental_results}
  \begin{tabular}{lcccc}
    \toprule
    \textbf{Annotation Type} & \multicolumn{3}{c}{\textbf{Score per
    Class}} & \textbf{Mean Dice} \\
    \cmidrule(lr){2-4}
    & \textbf{Other} & \textbf{Tumor} & \textbf{Stroma} & \\
    \midrule
    Scalar & 0.469 & 0.665 & 0.473 & 0.340 \\
    \midrule
    Features & 0.534 & 0.673 & 0.490 & \textbf{0.554} \\
    \midrule
    Pixel & 0.015 & 0.324 & 0.281 & 0.415 \\
    \bottomrule
  \end{tabular}
\end{table}