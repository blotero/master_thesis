...
\subsubsection{Centroid penalty}
...
adding to one. This was found useful in situations in which the
...
\begin{equation}
  ...
  \mathbb{E}[( \Lambda_r - 1)^2]
\end{equation}

The behavior of this regularization term is shown in Figure
\ref{fig:regularization_terms} (c).

\subsubsection{Missing labels penalty}
  ...
  \begin{subfigure}[b]{0.48\textwidth}
    \begin{tikzpicture}
      \begin{axis}[
          axis lines=middle,
          xlabel={$\Lambda_r$},
          ylabel={$\mathcal{L}_{sum}^r(\Lambda_r)$},
          width=\textwidth, height=6cm,
          domain=0:2,
          samples=200,
          smooth,
      ...
      \end{axis}
    \end{tikzpicture}
    \caption{$\mathcal{L}_{sum}^r(\Lambda_r) = (\Lambda_r - 1)^2$}
  ...
  \begin{subfigure}[b]{0.48\textwidth}
    \begin{tikzpicture}
      \begin{axis}[
        ...
        \addplot[blue,thick] {max(0, (1/(1-\alphaval))*(x)*exp((x -
        1)/\betaval))};
      \end{axis}
    \end{tikzpicture}
    \caption{$\mathcal{L}_{\lambda}^r(z) = \gamma \max(0,
    \frac{1}{1-\alpha_{\lambda}} z \cdot e^{\frac{z-1}{\beta}})$}
