...
\begin{table}[h!]
  ...
  \begin{tabular}{lcc}
    \hline
    \textbf{Method} & \textbf{Dice} & \textbf{Jaccard} \\
    \hline
    TGCE scalar & 0.8564 & 0.7691 \\
    TGCE pixel  & 0.8334 & 0.7392 \\
    \textbf{STAPLE}      & \textbf{0.8639} & \textbf{0.7795} \\
    MV          & 0.8214 & 0.7351 \\
    \hline
  \end{tabular}
  \caption{Performance comparison of methods using Dice and Jaccard
    metrics for the Oxford-IIIT Pet Dataset with no labelers
    aggregation. STAPLE outperforms all other methods, followed by
  TGCE$_{SS}$ scalar and pixel. MV performs the worst.}
...
While STAPLE achieves the highest Dice and Jaccard scores (0.8639 and
0.7795 respectively), our proposed TGCE$_{SS}$ approaches show
competitive performance, with the scalar variant achieving a Dice
score of 0.8564 and Jaccard of 0.7691. The pixel-wise variant, while
slightly lower in performance metrics, provides more detailed spatial
reliability information.

It's important to note that these results, when compared directly
with STAPLE and Majority Voting (MV) without labeler aggregation,
show that STAPLE maintains its advantage in terms of pure
segmentation accuracy. However, this comparison alone doesn't fully
capture the value of our approach. The key strength of our method
lies in its ability to simultaneously learn both segmentation and
reliability estimation, providing valuable insights into annotator
behavior and confidence levels that STAPLE and MV cannot offer.
