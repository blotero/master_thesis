...
\begin{figure}[h]
  ...
  \caption{Original U-NET architecture from \cite{Ronneberger2015}.}
...
of the terms from the proposed loss function penalizes
...
reliability of the labeler $i$ for a certain patch and
$\mathbb{LM}_i$ is the one hot label mask of the
...
estimating the reliability of the labelers. This leads to an special overall
loss function for the TNBC dataset of the form:

$$
\mathcal{L} = \mathcal{L}_{GCE} + \gamma \mathcal{L}_{\lambda}
\label{eq:tnbc_loss_function}
$$

where $\gamma$ is a hyperparameter that controls the trade-off between
the GCE loss and the regularizer.
