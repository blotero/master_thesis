...
\subsection{Sum-to-one regularization}

The reliability branch for scalar, pixel and features approaches,
might or might not
be forced to sum to one depending on the output activation function
\footnote{This
  was left as flexible and configurable in the Network Architecture
  Building API from seg\_tgce, allowing it to be either softmax or
sigmoid.}. For the cases in which this sum is not
forced to be unitary (sigmoid) the flexibility of this sum is manageable
through a regularization term which mildly penalizes the model for adding
to values too far from the unit. This was found useful in situations
in which the reliability maps were not expected to be punctuated (e.g. for the
...
similar levels of expertise. \footnote{The behavior of this
  regularization term of focusing in the homogeneity of the
  reliability maps is mostly covered by the combination of the
  centroid and entropy regularization terms, which was proven to be
redundant in section \ref{sec:pretraining-study}.}

\begin{equation}
  \mathcal{L}_{sum} = \alpha_{sum} \cdot
  \left(\sum_{\forall r \in R} \Lambda_r - 1  \right)^2
\end{equation}
...
\begin{figure}[H]
  ...
  \begin{subfigure}[b]{0.48\textwidth}
    \begin{tikzpicture}
      \begin{axis}[
          axis lines=middle,
          xlabel={$\Lambda_r$},
          ylabel={$\mathcal{L}_{sum}(\sum \Lambda_r)$},
          width=\textwidth, height=6cm,
          domain=0:2,
          samples=200,
          smooth,
        ...
        % Plot: (x - 1)^2
        \addplot[purple,thick] {(x - 1)^2};
      \end{axis}
    \end{tikzpicture}
    \caption{$\mathcal{L}_{sum}^r(\sum \Lambda_r) =
    (\sum \Lambda_r - 1)^2$}
    \label{fig:sum_to_one_regularization}
  \end{subfigure}
  \hfill
  \begin{subfigure}[b]{0.48\textwidth}
    \begin{tikzpicture}
  ...
  \caption{Visualization of the four regularization terms used in the
    TGCE loss function:
    (a) centroid penalty for extreme reliabilities, (b) entropy regularization,
    (c) sum-to-one regularization, and (d) missing labels penalty.
    Each subplot shows
...
\begin{align}
  ...
  &+\alpha_{reg} \cdot \mathbb{E}_r[(\Lambda_r - 0.5)^2] \nonumber \\
  &+\alpha_{entropy} \cdot \mathbb{E}_r[\Lambda_r \log(1 + \Lambda_r)
  +(1 - \Lambda_r) \log(1 + (1 - \Lambda_r))] \nonumber \\
  &+\alpha_{sum} \cdot (\sum_{\forall r \in R} \Lambda_r -
  1)^2 \nonumber \\
