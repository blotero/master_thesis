...
still rely on Majority Voting at some stage. In \cite{Elnakib2020},
a majority voting strategy is used for delivering a final output
based on the labels of multiple models (VGG16-Segnet, Resnet-18 and
Alexnet) in \gls{CT} images for Liver Tumor Segmentation.
Majority voting as a technique for label aggregation is a powerful
approach for its simplicity in many use cases in which the target to
be labeled is not tied to an expertise related task, otherwise, the
assumption of equal expertise among the labelers may introduce
bias in the final label, which is not desirable in the
case of highly technical annotations like medical images. In
subsection \ref{subsec:expertise_levels_lit_review}, a review is done on
literature which no longer assumes the naive approach of equal
expertise among labelers and face the challenge of learning from
inconsistent labels will be presented.
...
binary and multiclass classifier. The crowdsourcing
...
\begin{equation}\label{eq:staple_likelilhood}
  (\hat{\mathbf{p}}, \hat{\mathbf{q}}) = \arg\max_{\mathbf{p}, \mathbf{q}} \ln
  f(\mathbf{D},
...
maximize the log likelihood function, which is done iteratively
with step computations:

\begin{equation}
  \label{eq:staple_em_step}
  \begin{split}
...
The weight terms $W_i^{(k-1)}$ and $(1 - W_i^{(k-1)})$
represent the current estimates of the probability that voxel $i$
belongs or does not belong to the true segmentation from the previous
E-step. The log-likelihood contributions $\ln p_j$, $\ln (1 - q_j)$,
$\ln (1 - p_j)$, and $\ln q_j$ account for true positives, false
positives, false negatives, and true negatives, respectively,
weighted by the current estimate of true segmentation probabilities.
...
\gls{EM} approach. Additionally, STAPLE cannot correct systematic biases in
...
Finally, while \gls{STAPLE} proves valuable for aggregating existing
labels and supporting other methodologies, its primary limitation
lies in its inability to generate annotations for novel, unlabeled
images. This restricts its applicability to scenarios where multiple
annotations already exist, rather than serving as a standalone
solution for new image segmentation tasks.
...
segmentation \cite{KrithikaEtAl2022}.
...
image and the one-hot encoded identifiers of the labelers and outputs
a reliability map across the image feature space. This map is then
used to weight the contribution of each annotator to the final segmentation.
The second network is the U-Net used for segmentation.

In this approach, it is assumed that the images are labeled for at least one
labeler and not all of them, which is closer to a real world scenario, in
which images with variability in the amount of annotations per patch
is common. Hence, the input data can be modeled as:
...
\begin{itemize}
  \item \gls{CR} global: a single reliability vector per labeler with
    dimensions $C$ which represent global reliability of the labeler
    across all input space.
  \item \gls{CR} image: a single reliability vector per image per labeler
    with dimensions $C$ which represent local reliability of the labeler
    across the image.
  \item \gls{CR} pixel: a reliability matrix per image per labeler,
    with dimensions $C$ which represent local reliability of the labeler
...
Let $\mathbf{p}_\theta(\vecx_n)$ be the estimated latent (ground
truth) segmentation produced by the segmentation U-Net network. The
estimated segmentation probability mask for each annotator is then
...
(0.7039) and matching expert-supervised performance (0.7723). The \gls{CR}
image reliability modeling proved most effective, on the other hand
\gls{CR} pixel, while potentially offering finer-grained reliability
estimation, requires significantly more training data.

\begin{figure}
  \centering
  \includegraphics[width=0.85\textwidth]{Cap1/Figures/lopez_2024_proposed_framework.png}
  \caption{Proposed framework for the approach in \cite{LopezEtAl2024}.}
  \label{fig:lopez_2024_proposed_framework}
\end{figure}

Despite the proven performance of the approach by the authors,
solving the problem of multiple labelers with two networks can be
overwhelming for the optimization process, requiring large amounts of
annotated data to properly codify the annotators spatial
reliabilities, which could be managed by a single model with an
...
authors propose a novel approach based on Gaussian Processes for
modeling the relationship between the annotators' reliability and the
input data, while also preserving the interdependencies among the
annotators. This is achieved by introducing \gls{CCGPMA}, a framework
based on the well known \gls{CGP}. \gls{CGP} on itself cannot consider
inter-annotator dependencies, thus, the authors introduce the \gls{CCGP}
...
classification and regression, reaching the only known approach to
...
among the annotators, however, it does not face the image
segmentation problem, which is the main focus of this
...
\gls{MV} or \gls{EM} have been widely used for aggregating multiple
annotators' inputs. However, they assume a homogeneous reliability of
annotators, which may not hold in real-world scenarios.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{p{0.2\textwidth}p{0.3\textwidth}p{0.3\textwidth}p{0.15\textwidth}}
    ...
    \midrule
    \gls{MV} \cite{LamAndSuen1997} & Simple aggregation
    of multiple annotators' labels &
    - Simple to implement
    ...
    \midrule
    STAPLE \cite{WarfieldEtAl2004} & Probabilistic framework
    estimating true segmentation and
    rater reliability &
    ...
    \midrule
    U-Net based approaches \cite{RonnebergerEtAl2015} & Deep Learning
    architecture with
    ...
    \midrule
    Bayesian Models (CCGPMA) \cite{GilGonzalezEtAl2023} & Gaussian
    Processes modeling annotator
    reliability &
    ...
    - Complex optimization process \\
    \midrule
    Generalized Cross-Entropy (GCE) \cite{ZhangEtAl2018} & Loss
    function balancing MAE and CE &
