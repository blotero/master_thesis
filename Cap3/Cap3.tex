...
\subsection{Kernel Alignment-Based Annotator Relevance Analysis}

The \gls{KAAR} method, introduced in \cite{GilGonzalezEtAl2018},
...
multiple annotators $\mat{Y}$, \gls{KAAR} computes a kernel matrix
...
A key advantage of \gls{KAAR} is its ability to capture dependencies
...
In \cite{GilGonzalezEtAl2021} the authors extend \gls{KAAR} by introducing
locality in the kernel alignment framework. Rather than assuming
constant annotator performance across the input space, \gls{LKAAR} models
the annotator relevance as a function of the input features. The
kernel combination in \gls{LKAAR} takes the form:
...
This localized approach allows \gls{LKAAR} to better model inconsistent
annotators whose performance varies across different regions of the
input space \cite{GilGonzalezEtAl2021}.

\subsection{Chained Gaussian Processes Model}

The \gls{CGP} model provides a probabilistic framework for modeling
multiple outputs through a chain of Gaussian processes \cite{SaulEtAl2016}.
In the context of multiple annotators, \gls{CGP} can be used to model
both the ground truth and annotator behavior. The key idea is to model
a likelihood function with $J$ parameters as in \cite{GiraldoEtAl2022}:
...
modeling complex relationships within the data \cite{GiraldoEtAl2022}.
The use of Gaussian processes enables natural uncertainty quantification through
...
most important features for prediction \cite{GiraldoEtAl2022}.

The \gls{CGP} framework becomes particularly powerful when integrated with
concepts from \gls{KAAR} and \gls{LKAAR}. By incorporating kernel alignment
principles, the model can make more informed choices about kernel
functions, better capturing the underlying structure of the data
\cite{GiraldoEtAl2022}. The framework can be extended to include
input-dependent lengthscales, allowing for varying degrees of smoothness
across the input space \cite{GiraldoEtAl2022}. Furthermore, through
multi-output \gls{GP} formulations, the model can capture complex dependencies
capture complex dependencies between different annotators, providing
a rich representation of how different experts' opinions relate to
each other.

However, implementing \glspl{CGP} in practice presents several significant
...
for large-scale applications \cite{LiuEtAl2020}. The need for
approximate inference techniques in non-Gaussian likelihood scenarios
can introduce additional complexity and potential approximation
errors \cite{RasmussenEtAl2006Chapter3}.
When dealing with high-dimensional inputs, the model
may struggle with the curse of dimensionality, requiring careful
feature selection or dimensionality reduction preprocessing
\cite{RasmussenEtAl2006Chapter3}.
Additionally, the selection of appropriate kernel functions remains a
critical challenge, as it significantly impacts the model's
performance and its ability to capture relevant patterns in the data
\cite{RasmussenEtAl2006Chapter9}.

\subsection{Regularized Chained Deep Neural Network}

In \cite{GilGonzalezEtAl2021-RCDNN}, the authors propose a deep
learning approach to multiple annotator learning, inspired by the
chained Gaussian processes model. The method models both the ground
truth estimation and annotator reliability through a deep neural
network architecture. The likelihood function in \gls{RCDNN} takes the form:
...
The \gls{RCDNN} architecture is built upon a sophisticated multi-layer
structure that combines several key innovations in deep learning
\cite{GilGonzalezEtAl2021-RCDNN}. At its core, it employs multiple dense
layers with carefully chosen activation functions, each designed to
capture different aspects of the annotator-data relationship. To prevent
overfitting, a comprehensive regularization strategy is implemented,
combining both L1 and L2 norms with dropout mechanisms
\cite{GilGonzalezEtAl2021-RCDNN}. The network architecture
...
the complexity of deep architectures \cite{SaulEtAl2016}.
In this work, two key frameworks are leveraged: GPflow and
GPflux. These tools provide the foundation for building scalable and
efficient implementations of our models while maintaining the
theoretical rigor necessary for proper uncertainty quantification.
...
they offer a modular design that allows for
easy extension and modification, which is crucial when implementing
novel model architectures. Third, they provide robust
implementations of various Gaussian process models and inference
methods, allowing us to focus on the specific challenges of multiple
annotator learning rather than reimplementing basic GP functionality.
...
process models including \gls{GPR}, \gls{GPMC}, \gls{SGPR}, and
\gls{SGPMC} \cite{MatthewsEtAl2016}. These implementations are built on top of
TensorFlow's computational graph framework, allowing for automatic
differentiation and GPU acceleration where available.
...
training, making it practical to work with datasets that don't fit in
memory.
...
\begin{equation}\label{eq:classification_targets}
  \begin{aligned}
    t_{n,1} &= \sin(2\pi x_n), \\
    t_{n,2} &= -\sin(2\pi x_n), \\
...
nonlinear decision boundary between the classes \cite{GilEtAlvarez2023CCGPMA}.
...
annotator labels as corrupted versions of the hidden ground truth.
In these experiments, the simulation is
designed to account for: 1) dependencies among annotators, and 2)
labeler performance that varies as a function of the input features.
...
the inverse exponential sigmoid function.
...
\begin{equation}\label{eq:regression_targets}
...
\section{Results}\label{sec:gps_results}

The built synthetic datasets are used to train the \gls{CCGPMA}
model. Emulated classification labels following the true label
determining function from \ref{eq:classification_targets} can be seen in
Figure \ref{fig:classification_labels}. On the other hand, the ground
truth and noisy labels emulation for annotators across the feature
space for regression task can be seen in Figure \ref{fig:regression_labels}.

Using both GPflux and GPflow for the regression and classifications
tasks, leads to the following combinations experiments: GPflux-CCGPMA
Regression, GPflow-CCGPMA Regression, GPflux-CCGPMA Binary
Classification, GPflow-CCGPMA Binary Classification, GPflow-CCGPMA
Multiclass Classification, GPflux-CCGPMA Multiclass Classification.
All of the experiments converged successfully with almost identical
results and close to no difference in performance.

\begin{figure}
\centering
\begin{subfigure}[b]{0.8\textwidth}
\includegraphics[width=\textwidth]{Cap3/Figures/multiclass_ma_ground_truth.png}
\caption{Ground truth labels across the feature space}
\label{fig:classification_labels_ground_truth}
\end{subfigure}
\begin{subfigure}[b]{0.8\textwidth}
\includegraphics[width=\textwidth]{Cap3/Figures/multiclass_noisy_emulations.png}
\caption{Noisy emulations for annotators across the feature space for
classification task}
\label{fig:classification_labels_noisy_emulations}
\end{subfigure}
\caption{Synthetic dataset overview for classification task}
\label{fig:classification_labels}
\end{figure}

\begin{figure}
\begin{subfigure}[b]{0.4\textwidth}
\includegraphics[width=\textwidth]{Cap3/Figures/multiclass_elbo_gpflux.png}
\caption{ELBO convergence}
\label{fig:elbo_convergence_gpflux}
\end{subfigure}
\begin{subfigure}[b]{0.4\textwidth}
\includegraphics[width=\textwidth]{Cap3/Figures/multiclass_ma_test_gpflux.png}
\caption{Trained CCGPMA predictions on the test set}
\label{fig:classification_test_gpflux}
\end{subfigure}
\caption{Classification results for the \gls{CCGPMA} model using GPflux.}
\label{fig:classification_results_gpflux}
\end{figure}

\begin{figure}
\centering
\includegraphics[width=0.7\textwidth]{Cap3/Figures/regression_labels.png}
\caption{Ground truth and noisy labels emulation for annotators
across the feature space for regression task.}
\label{fig:regression_labels}
\end{figure}

\begin{figure}
\centering
\begin{subfigure}[b]{0.5\textwidth}
\includegraphics[width=\textwidth]{Cap3/Figures/regression_gpflux_results.png}
\caption{Trained CCGPMA predictions on the test set, for inference of both the
hidden ground truth and the annotators' reliabilities across the feature space.}
\label{fig:regression_results}
\end{subfigure}
\caption{Regression results for the \gls{CCGPMA} model using GPflux.}
\end{figure}
...
from diverse expert opinions. \gls{KAAR} established a
foundation with its non-parametric approach through kernel alignment,
though it operates under the assumption of constant annotator
performance across the input space. \gls{LKAAR} advanced this concept
by introducing input-dependent annotator behavior, recognizing that
expert performance often varies across different types of inputs.
\gls{RCDNN} brought the power and flexibility of deep learning to the
problem, offering excellent scalability but requiring careful attention
to regularization and architecture design. \gls{CCGPMA} represents perhaps
the most theoretically elegant approach, providing a principled
...
annotator behavior could make \gls{LKAAR} or \gls{RCDNN} more appropriate
choices. Additionally, when interpretability is a key requirement, as
in many medical applications, methods like \gls{KAAR} and \gls{LKAAR}
might be preferred over the more complex Deep Learning approaches.
...
\begin{tabular}{lcccc}\toprule
  ...
  \textbf{RCDNN} & \textbf{CGP} \\
  & \cite{GilGonzalezEtAl2018} & \cite{GilGonzalezEtAl2021} &
  \cite{GilGonzalezEtAl2021-RCDNN} & \cite{GilEtAlvarez2023CCGPMA} \\\midrule
...
\end{tabular}}
\vspace{0.1cm}
{\small{\textbf{Note:} Partial uncertainty quantification in \gls{RCDNN}
...
interdependencies \cite{GilEtAlvarez2023CCGPMA}. This requires
extending the basic \gls{GP} layers
...
Third, kernel selection and design presents a critical challenge in
implementing the CGP model. The choice of kernel functions significantly
impacts the model's ability to capture the underlying structure of the
data and the relationships between annotators \cite{RasmussenEtAl2006Chapter9}.
Several considerations must be taken into account:

\begin{itemize}
\item \textbf{Kernel Complexity}: The model requires multiple kernel
functions to capture different aspects of the data - one for the
ground truth and additional kernels for each annotator's reliability
model. This multi-kernel setup increases the complexity of kernel
selection and optimization.

\item \textbf{Input-Dependent Behavior}: Since annotator performance
often varies across the input space, the kernel functions must be
flexible enough to capture this non-stationary behavior. This may
require using composite kernels or kernels with input-dependent
lengthscales.

\item \textbf{Inter-Annotator Dependencies}: The kernel design must
account for potential correlations between different annotators'
behaviors. This can be achieved through careful design of the kernel
structure in the SLFM framework, where the combination of basis
kernels must be chosen to effectively model these dependencies.

\item \textbf{Computational Efficiency}: The choice of kernel
functions also impacts computational performance. While more
expressive kernels may better capture complex patterns, they often
come with increased computational cost. This trade-off must be
carefully considered, especially when dealing with large datasets.
\end{itemize}
...
While Gaussian Processes have demonstrated remarkable success in
regression and classification tasks, their application to image
segmentation presents significant computational challenges
\cite{RasmussenEtAl2006Chapter9}. The cubic
computational complexity of GPs ($\mathcal{O}(N^3)$), which scales with
the number of data points, becomes particularly problematic when
dealing with high-resolution images where each pixel can be considered
a data point \cite{LiuEtAl2020}.
This limitation makes traditional GP approaches
impractical for large-scale image segmentation tasks, where the number
of pixels can easily reach into the millions. Alternative approaches,
such as deep learning-based segmentation methods, often provide more
computationally efficient solutions for these high-dimensional
problems, though at the cost of uncertainty quantification
capabilities that GPs naturally provide \cite{KendallEtAl2017}
\cite{WilsonEtAl2016}.
